...
var app = angular.module('myApp', []);
	app.controller('listController', function ($scope, $http) {
		...
		var postData = {type: 'searchtype',falg:"1"};
		...
		$scope.save = function (type) {
	    	...
	    	var obj = document.getElementById('datalist').elements;
	    	$scope.datelist = [];
	    	$scope.pricelist = [];
	    	$scope.marketpricelist = [];
	    	$scope.idlist = [];
	    	$scope.detailid = [];
	    	...
	    	for(var i=0;i<obj.length;i++){
	    		if(obj[i].name=="date"){
	    			$scope.datelist.push(obj[i].value);
	    		}
	    		if(obj[i].name=="aduit_price"){
	    			$scope.pricelist.push(obj[i].value);
	    		}
	    		if(obj[i].name=="market_aduitprice"){
	    			$scope.marketpricelist.push(obj[i].value);
	    		}
	    		if(obj[i].name=="rowid"){
	    			if(obj[i].value!=""){
	    				$scope.idlist.push(obj[i].value);
	    			}
	    		}
	    	}
	    	...
	    	if(type=="modify"){
	    		var postData = {
    	            ...
    	            state:1,
    	            valid:1,
    	            datelist:$scope.datelist,
    	            pricelist:$scope.pricelist,
    	            marketpricelist:$scope.marketpricelist,
    	            idlist:$scope.idlist,
...
function adddaterows() {
    ...
    var market_aduitprice = $("#money1").val();
    if(aduit_price==""){
...
function addrow(i,j) {
    ...
    var trhtml = '<div class="combo-item" id="list' + j + '" >' +
            ...
            '</div></div></div>' +
            '<div class="con-table"><label class="layui-form-label">第&nbsp;' + j + '&nbsp;天&nbsp;&nbsp;</label><textarea name="lineday' + i + '" id="lineday' + i + '" style="width:700px;height:200px;visibility:hidden;" rows="8"></textarea></div></div>';
